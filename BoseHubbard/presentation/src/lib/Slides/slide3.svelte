...
<Slide>
	...
	<Slide transition="fade">
		...
		<div class="r-stack">
			...
			<div class="fragment center current-visible" data-fragment-index="0">
                ...
                `}$$ How to obtain the phase diagram of the Bose-Hubbard model in the (${`t/U, \\mu/U`}$)
				plane ?
			</div>
			<div class="fragment center current-visible" data-fragment-index="1">
				...
				<p class="text-left flex flex-col space-y-4">
					${`a_i = \\langle a_i \\rangle + \\delta_{a_i}`}$ ${`a_i^\\dagger = \\langle a_i^\\dagger \\rangle + \\delta_{a_i^\\dagger}`}$
					${`\\langle a_i \\rangle = \\langle a_i^\\dagger \\rangle ^\\star = \\Phi_i`}$ ${`\\delta_{a_i}\\delta_{a_i^\\dagger} = 0`}$
				</p>
			...
			<div class="fragment center current-visible" data-fragment-index="2">
                ...
                `}$$ ${`\\rightarrow`}$ Find ${`\\Phi`}$ which minimizes the free energy of the
				system.
			</div>
		</div>
	</Slide>
	<Slide transition="fade">
		...
		<ol>
			<li>
				Generate ${`\\mathcal{H}_{\\text{MF}}`}$ for a ${`(t,\\mu)`}$ pair value and an initial
				guess ${`\\Phi_{\\text{init}}`}$ (${`=1`}$) ;
			</li>
			<li>
				Diagonalize ${`\\mathcal{H}_{\\text{MF}}`}$ to find the lowest eigenvalue ${`\\epsilon_0`}$
				and associated eigenvector ${`\\phi_0`}$ ;
			</li>
